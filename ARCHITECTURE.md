...
上下文管理使用确定性的字符预算策略：系统提示和原始任务始终保留，优先保留最近的完整工具交互，较早交互压缩成摘要。特色功能是 `update_plan` 本地工具，模型可以创建计划、更新当前步骤并标记已完成步骤，计划状态会在后续请求中注入系统提示。Agent 默认最多执行 20 轮模型请求，同时保留重复工具失败熔断；若连续重复相同工具调用，或连续多轮只检查文件而没有运行测试，会向模型发送进展提醒。
